...
if (!malls_500m_gpkg %in% dir ())
{
    ...
    malls_5000m <- malls %>% st_buffer (5000) %>% st_union %>%
        st_cast ("POLYGON")
    st_write (malls_500m, malls_500m_gpkg)
    st_write (malls_5000m, malls_5000m_gpkg)
}
malls_500m <- st_read (malls_500m_gpkg)
malls_5000m <- st_read (malls_5000m_gpkg)
...
for (i in seq_along (shop_classes))
{
    ...
    {
        gpkg_name <- paste0 ("shop_", shop_classes [i], "_", vals [j],
                             ".gpkg")
        if (!gpkg_name %in% dir ())
        {
            ...
            if (class (sh) != "try-error")
            {
                ...
                if (dim (sh) [1] > 0)
                {
                    ...
                    st_write (sh, gpkg_name, update = TRUE)
                }
            }
        }
    }
...
if (!junctions_gpkg %in% dir ())
{
    ...
    st_write (junctions, junctions_gpkg)
}
        ...
        if (!gpkg_name %in% dir ())
        {
            ...
            if (class (am) != "try-error")
            {
                ...
                if (dim (am) [1] > 0)
                {
                    ...
                    st_write (am, gpkg_name, update = TRUE)
                }
            }
        }
    }
...
if (!highways_gpkg %in% dir ())
{
    ...
    st_write (highways, highways_gpkg)
}

shops <- st_read (shops_gpkg)
public_transport <- st_read (public_transport_station_gpkg)
highways <- st_read (highways_gpkg)
junctions <- st_read (junctions_gpkg)
amenities <- st_read (amenities_gpkg)
```
