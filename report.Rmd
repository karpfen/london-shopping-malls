...
if (!results_gpkg %in% dir (dir_out))
{
    ...
    res_stats <- rbind (res_min, res_mean, res_median, res_max)
    colnames (res_stats) <- cnames_res
    rownames (res_stats) <- c ("min", "mean", "median", "max")
...
```{r results = 'asis', warnings = FALSE, message = FALSE}
aoi_wkt <- paste0 ("POLYGON((-0.510375 51.691875, 0.334015 51.691875, ",
                   "0.334015 51.286758, -0.510375 51.286758, ",
                   "-0.510375 51.691875))")
aoi <- st_as_sfc (aoi_wkt) %>% st_set_crs (4326) %>%
    st_transform (crs_utm_london)
aoi_sp <- as (aoi, "Spatial") %>% geometry
pts <- spsample (aoi_sp, type = "hexagonal", cellsize = 1000, offset = c (0, 0))
hexgrid <- HexPoints2SpatialPolygons (pts) %>% as ("sf")

cnames_hex <- c ( "occurrence_shops", "occurrence_amenities",
                  "distribution_shop", "distribution_amenities",
                  "proximity_shops", "proximity_amenities",
                  "proximity_transport", "correlation_shops_amenities")
...
if (!results_hex_gpkg %in% dir (dir_out))
{
    ...
    {
        cell <- hexgrid [i, ]
        amenities_i <- clip_pts_by_poly (amenities, cell)
        public_transport_i <- clip_pts_by_poly (public_transport, cell)
        shops_i <- clip_pts_by_poly (shops, cell)

        res_hex [i, 1] <- tc (occurrence (shops_i))
        res_hex [i, 2] <- tc (occurrence (amenities_i))
        res_hex [i, 3] <- tc (distribution (shops_i))
        res_hex [i, 4] <- tc (distribution (amenities_i))
        res_hex [i, 5] <- tc (proximity (cell, shops_i))
        res_hex [i, 6] <- tc (proximity (cell, amenities_i))
        res_hex [i, 7] <- tc (proximity (cell, public_transport_i))
        res_hex [i, 8] <- tc (correlation (shops_i, amenities_i))
    }
    ...
    rownames (res_hex_stats) <- c ("min", "mean", "median", "max")
