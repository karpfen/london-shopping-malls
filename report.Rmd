...
res <- matrix (FALSE, nrow = nrow (malls), ncol = 7) %>% as.data.frame
names (res) <- c ("occurence_shops", "occurence_amenities",
                  "distribution_shops", "distribution_amenities",
                  "proximity_transport", "proximity_junction",
                  "proximity_highway")

for (i in seq_len (nrow (malls)))
{
    mall <- malls %>% extract (i, )
    mall_500m <- malls_500m %>% extract (i, ) %>% extract ("geom")
    mall_5000m <- malls_5000m %>% extract (i, ) %>% extract ("geom")

    amenities_i <- clip_pts_by_poly (amenities, mall_500m)
    ...
    shops_i <- clip_pts_by_poly (shops, mall_500m)

    res [i, 1] <- tc (occurence (shops_i, 40, 0))
    res [i, 2] <- tc (occurence (amenities_i, 40, 0))
    res [i, 3] <- tc (distribution (shops_i, c (-0.1, Inf)))
    res [i, 4] <- tc (distribution (amenities_i, c (-0.1, Inf)))
    res [i, 5] <- tc (proximity (mall, public_transport_i, c (0, 250)))
    res [i, 6] <- tc (occurence (junctions_i, 0, 2000))
    res [i, 7] <- tc (occurence (highways_i, 0, 2000))
    res <- ifelse (res == TRUE, 1, 0) %>% as.data.frame
}
