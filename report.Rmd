...
res <- matrix (NA, nrow = nrow (malls), ncol = 9) %>% as.data.frame
names (res) <- c ("occurence_shops_500m", "occurence_amenities_500m",
...
for (i in seq_len (nrow (malls)))
{
    ...
    res [i, 5] <- tc (distribution (shops_500m_i))
    res [i, 6] <- tc (distribution (amenities_500m_i))
    res [i, 7] <- tc (proximity (mall, public_transport_i))
    res [i, 8] <- tc (occurence (junctions_i))
    res [i, 9] <- tc (occurence (highways_i))
}
